...
6. Выполняется проверка условий шлюза: если общее число уникальных сущностей (участники + упоминания) меньше 50, генерируется текстовый список и отправляется пользователю как обычное сообщение, если больше или равно 51 — строится Excel‑отчёт, временно сохраняется в памяти и отсылается как документ, после чего временные данные очищаются.
...
Сервис обработки обновлений (chat‑processing) выступает оркестратором над тремя основными сервисами: парсером экспорта, сервисом анализа чатов и генератором отчётов; он получает доменные объекты от парсера, передаёт их в анализ и затем, в зависимости от общего количества сущностей (участники + упоминания), выбирает формат ответа: компактный текстовый список или Excel‑файл.
